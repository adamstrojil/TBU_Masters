...
export default function TabTwoScreen() {
  const [affirmationList, setAffirmationList] = useState<Array<string>>([]);
  ...
  useEffect(() => {
    isFocused &&
      getMyObject(DATABASE_KEY).then(({ affirmations }) => {
  ...
  useEffect(() => {
    setFilteredResults(
      query
        ? affirmationList.filter((affirmation) =>
            affirmation.toLowerCase().includes(query.toLowerCase())
          )
        : affirmationList
    );
  }, [query]);
  ...
  return (
    <View style={styles.container}>
      ...
      {!query && !!affirmationList.length && (
        <Button title="Clear list" onPress={clearAll} color="red" />
      )}
    </View>
  );
}
